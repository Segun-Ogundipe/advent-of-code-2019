import { puzzleInput } from './data/puzzle-input';
/**
 * Returns an integer as the result of a gravity assist algorithm.
 *
 * @param intCode - An intcode array
 */
function gravityAssist1(intCode: number[]): number {
  for (let i = 0, len = intCode.length; i < len; i++) {
    const opCode: number = intCode[i];
    if (opCode == 1) {
      intCode[intCode[i + 3]] = intCode[intCode[i + 1]] + intCode[intCode[i + 2]];
      i += 3;
    } else if (opCode == 2) {
      intCode[intCode[i + 3]] = intCode[intCode[i + 1]] * intCode[intCode[i + 2]];
      i += 3;
    } else if (opCode == 99) {
      return intCode[0];
    }
  }

  return -1;
}

/**
 * Returns the output gravity of a gravity assist algorithm.
 *
 */
function gravityAssist2(): number {
  let output = 0;
  for (let noun = 0; noun <= 99; noun++) {
    for (let verb = 0; verb <= 99; verb++) {
      let newIntCode: number[] = [
        1,
        12,
        2,
        3,
        1,
        1,
        2,
        3,
        1,
        3,
        4,
        3,
        1,
        5,
        0,
        3,
        2,
        9,
        1,
        19,
        1,
        19,
        5,
        23,
        1,
        23,
        6,
        27,
        2,
        9,
        27,
        31,
        1,
        5,
        31,
        35,
        1,
        35,
        10,
        39,
        1,
        39,
        10,
        43,
        2,
        43,
        9,
        47,
        1,
        6,
        47,
        51,
        2,
        51,
        6,
        55,
        1,
        5,
        55,
        59,
        2,
        59,
        10,
        63,
        1,
        9,
        63,
        67,
        1,
        9,
        67,
        71,
        2,
        71,
        6,
        75,
        1,
        5,
        75,
        79,
        1,
        5,
        79,
        83,
        1,
        9,
        83,
        87,
        2,
        87,
        10,
        91,
        2,
        10,
        91,
        95,
        1,
        95,
        9,
        99,
        2,
        99,
        9,
        103,
        2,
        10,
        103,
        107,
        2,
        9,
        107,
        111,
        1,
        111,
        5,
        115,
        1,
        115,
        2,
        119,
        1,
        119,
        6,
        0,
        99,
        2,
        0,
        14,
        0
      ];
      newIntCode[1] = noun;
      newIntCode[2] = verb;
      output = gravityAssist1(newIntCode);
      if (output == 19690720) {
        return 100 * noun + verb;
      }
    }
  }

  return -1;
}

console.log(gravityAssist1(puzzleInput), gravityAssist2());

export { gravityAssist1, gravityAssist2 };
